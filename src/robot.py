...
class RRRRobot:
    ...
    def mass_matrix(self, q):
        ...
        I11 = m1*L1**2 + m2*(L1**2 + L2**2 + 2*L1*L2*np.cos(q2)) \
                    + m3*(L1**2 + L2**2 + L3**2 + 2*L1*L2*np.cos(q2) + 2*L2*L3*np.cos(q3) + 2*L1*L3*np.cos(q2+q3))
        I12 = m2*(L2**2 + L1*L2*np.cos(q2)) \
             + m3*(L2**2 + L3**2 + L1*L2*np.cos(q2) + 2*L2*L3*np.cos(q3) + L1*L3*np.cos(q2+q3))
        I13 = m3*(L3**2 + L2*L3*np.cos(q3) + L1*L3*np.cos(q2+q3))
        I22 = m2*L2**2 + m3*(L2**2 + L3**2 + 2*L2*L3*np.cos(q3))
        I23 = m3*(L3**2 + L2*L3*np.cos(q3))
        I33 = m3*L3**2
        M = np.array([[I11, I12, I13],
                      [I12, I22, I23],
                      [I13, I23, I33]])
        return M
    ...
    def gravity(self, q):
        ...
        g1 = g*( m1*(L1/2)*np.cos(q1)
               + m2*(L1*np.cos(q1) + (L2/2)*np.cos(q1+q2))
               + m3*(L1*np.cos(q1) + L2*np.cos(q1+q2) + (L3/2)*np.cos(q1+q2+q3)) )
        g2 = g*( m2*( (L2/2)*np.cos(q1+q2) )
